...
모든 한글 텍스트 컨테이너에 적용:
```css
word-break: keep-all;     /* 단어 중간 줄바꿈 방지 (필수) */
line-height: 1.7;         /* CJK 권장 행간 */
...
- [ ] `word-break: keep-all` 적용 (필수)
- [ ] `line-height: 1.7` 이상 (본문)
- [ ] `letter-spacing: -0.01em` ~ `-0.02em`
- [ ] 최소 폰트 크기: 10pt (8pt는 가독성 저하)
- [ ] 5열 이상 그리드 금지 (한글은 공간 필요)
- [ ] Hero 타이틀: 64pt 이하 권장 (72pt+ 오버플로우 위험)
- [ ] Bold 텍스트 사용 시 글자 수 5% 줄이기

### 글자 수 제한 가이드

| 텍스트 박스 너비 | 제목 (22-28pt) | 본문 (12-14pt) |
|----------------|---------------|---------------|
| 200pt | 8-10자 | 14-16자 |
| 300pt | 12-15자 | 21-25자 |
| 400pt | 16-20자 | 28-33자 |
| 500pt | 20-25자 | 35-42자 |
| 624pt (전체폭) | 25-31자 | 44-52자 |

### 한글 Bold 텍스트 주의사항

Malgun Gothic 볼드는 일반체보다 약 5% 넓음:
- 볼드 사용 시 글자 수를 5% 줄이거나
- 텍스트 박스 너비를 5% 늘릴 것

### CDN 확인

```html
<!-- 올바른 Pretendard CDN 링크 (철자 확인!) -->
